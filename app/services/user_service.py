...
class UserService:
    ...
    def create(self, schema: UserCreateRequestSchema) -> UserResponseSchema | None:
        try:
            ...
            if "unique constraint" in error_message.lower() and "email" in error_message.lower():
                self._raise_bad_request(ApiMessageEnum.USER_EMAIL_EXISTS.value)
    ...
    def _validate_password(self, password: str, confirm_password: str) -> None:
        ...
        if password_is_null:
            self._raise_bad_request(ApiMessageEnum.USER_PASSWORD_MANDATORY.value)
        elif invalid_password:
            self._raise_bad_request(ApiMessageEnum.USER_PASSWORDS_DO_NOT_MATCH.value)
